...
- `scripts/audit_phase_16.lean` — five new `#print axioms` entries
  for the Workstream-H declarations; three new concrete `example`
  bindings in the `NonVacuityWitnesses` section exercising the
  `_one_right` Prop discharge, the `tight_one_exists` witness, and
  the full H3 composition pipeline on a singleton-orbit model. Type-
  checking the script is the machine-checkable verification that
  each headline result accepts well-typed inputs.
