...
Historical record of every workstream landing, audit-pass cleanup,
and phase-completion log for the Orbcrypt formalization. This
document is a **frozen archive** of state-at-the-time; entries are
preserved verbatim from the moment of landing.

## How to read this document

* Entries are organised by **audit cycle** and **workstream
  identifier**. Within each workstream, the entry records the
  motivation, the files touched, the verification posture, and
  the patch-version impact.
* Cross-references to specific theorem names may use **identifiers
  current at the time of the entry**, which can drift with later
  renames or deletions. For example, `two_phase_correct` (renamed
  in W1 of the 2026-05-06 structural review to
  `canonical_agreement_under_two_phase_decomposition`) appears in
  earlier entries with its original name.
* Where a deletion has occurred (e.g., the deterministic-OIA chain
  removal scheduled for v0.4.0), historical entries that mention
  the deleted identifier are preserved unchanged. Consult the
  current
  [`docs/API_SURFACE.md`](../API_SURFACE.md) (or
  [`CLAUDE.md`](../../CLAUDE.md)) for the post-deletion shape of
  the headline tables.
* For the current state of the formalization, see
  [`docs/API_SURFACE.md`](../API_SURFACE.md).

## Relocation note

This document was created by Workstream 5.2 of the 2026-05-06
structural review (plan
`docs/dev_history/AUDIT_2026-05-06_STRUCTURAL_REVIEW.md` § 1 row 5).
It contains content extracted verbatim from `CLAUDE.md` lines
880–9100 of the pre-W5.2 state. The pre-W5.2 `CLAUDE.md` was 9,061
lines; the post-W5.2 `CLAUDE.md` is approximately 1,200 lines of
stable conventions plus pointers, and this document carries
the relocated historical content.

The relocation preserved every entry verbatim: line counts, axiom-
transparency entries, files-touched lists, naming-discipline
notes, verification-posture statements. Subsequent workstream
snapshots will land directly in this document, not in `CLAUDE.md`.
