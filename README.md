...
**Formalization Complete — Phases 1–15 Done**

Phase 15 (Decryption Optimisation) published the fast decryption
pipeline: a GAP reference implementation of four alternative
decapsulation strategies (two-phase cyclic + residual,
syndrome-based, deterministic orbit hash, and the full-backtracking
baseline) plus the Lean 4 correctness specification. The
post-landing audit confirmed that the fast and slow canonical forms
DIFFER (lex-min over G doesn't commute with the residual transversal
action), so the production correctness contract runs through
orbit-constancy of `FastCanonicalImage` rather than through the
strong `TwoPhaseDecomposition` predicate; the GAP code provides
`FastEncaps` to pair with `FastDecaps` for end-to-end consistency.
See [`docs/planning/PHASE_15_DECRYPTION_OPTIMIZATION.md`](docs/planning/PHASE_15_DECRYPTION_OPTIMIZATION.md),
[`implementation/gap/orbcrypt_fast_dec.g`](implementation/gap/orbcrypt_fast_dec.g),
the CI-grade benchmark CSV at
[`docs/benchmarks/phase15_decryption_quick.csv`](docs/benchmarks/phase15_decryption_quick.csv),
and the production CSV
[`docs/benchmarks/phase15_decryption.csv`](docs/benchmarks/phase15_decryption.csv)
(regenerated by `RunPhase15Comparison`).
...
| Public-Key Extension | `PublicKey/{ObliviousSampling, KEMAgreement, CommutativeAction, CombineImpossibility}` | Orbital randomizers, two-party KEM agreement, CSIDH-style `CommGroupAction` and `CommOrbitPKE`, combiner impossibility |
| Decryption Optimisation | `Optimization/{QCCanonical, TwoPhaseDecrypt}` | QC cyclic canonical-form abstraction, two-phase decomposition predicate, fast-path KEM correctness (Phase 15) |

### Build Stats

- 38 Lean source files + root import file
- ~280 public definitions and theorems, all with docstrings
...
| [docs/planning/PHASE_15_DECRYPTION_OPTIMIZATION.md](docs/planning/PHASE_15_DECRYPTION_OPTIMIZATION.md) | Phase 15 — fast decryption (two-phase, syndrome, orbit hash) plan and exit criteria |
