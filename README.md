...
decapsulation strategies (two-phase cyclic + residual, syndrome-based,
probabilistic orbit hash, and the full-backtracking baseline) plus the
Lean 4 correctness specification of the two-phase factorisation. See
[`docs/planning/PHASE_15_DECRYPTION_OPTIMIZATION.md`](docs/planning/PHASE_15_DECRYPTION_OPTIMIZATION.md),
[`implementation/gap/orbcrypt_fast_dec.g`](implementation/gap/orbcrypt_fast_dec.g),
and the benchmark CSV generated by `RunPhase15Comparison()` at
...
| 16 | `two_phase_correct` — fast (cyclic ∘ residual) canonical form agrees with full | `Optimization/TwoPhaseDecrypt.lean` | Zero custom axioms (`TwoPhaseDecomposition` is a hypothesis) |
| 17 | `two_phase_kem_correctness` — two-phase KEM decapsulation recovers the encapsulated key | `Optimization/TwoPhaseDecrypt.lean` | Zero custom axioms (`TwoPhaseDecomposition` is a hypothesis) |
