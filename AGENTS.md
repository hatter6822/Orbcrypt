...
Orbcrypt/
  ...
  Theorems/
    ...
    OIAImpliesCPA.lean              OIA ⇒ IND-1-CPA security
  KEM/
...
implementation/
  gap/
    ...
    orbcrypt_test.g                 Correctness test suite — 13 tests
...
scripts/
  ...
  audit_phase_16.lean               Consolidated #print axioms script (342 declarations + witnesses)
  audit_hypothesis_consumption.py   Hypothesis-consumption gate (CI gate #5)
  generate_test_vectors.lean        GAP–Lean test-vector generator
...
GroupAction.Canonical  GroupAction.Invariant
        ...
        GroupAction.CanonicalLexMin
              ...
              /        \              |
   Crypto.Security  Crypto.OIA   KEM.Encapsulate
        |               \         /        \
        |       Theorems.OIAImpliesCPA   KEM.{Correctness, Security}
        |
   Theorems.{Correctness, InvariantAttack}
        |
   Construction.{Permutation, HGOE, HGOEKEM}

   Mathlib.Probability.{PMF, Uniform}
        |
   Probability.{Monad, Negligible, Advantage, UniversalHash}
        |
   Crypto.{CompOIA, CompSecurity}

   KEM.Encapsulate + Construction.Permutation
        |
   KeyMgmt.{SeedKey, Nonce}

   AEAD.MAC ──► AEAD.{AEAD, Modes, CarterWegmanMAC}
...
.claude/settings.json               Claude Code session hook
docs/
  API_SURFACE.md                    Canonical "what the formalization delivers" reference
  DEVELOPMENT.md                    Master specification (~56KB)
  ...
  VERIFICATION_REPORT.md            End-to-end verification report
...
Several files are large (`docs/DEVELOPMENT.md` is ~56KB). Always page through
large files using `offset` and `limit` in ≤500-line chunks rather than reading
the whole file:
...
Known large files: `docs/DEVELOPMENT.md` (~1200 lines),
`docs/dev_history/formalization/FORMALIZATION_PLAN.md` (~400 lines), and the
...
1. **`lake build`** — full project builds clean (zero warnings, zero
   errors).
2. **No-`sorry` check** — comment-aware Perl strip + `grep` returns empty
   across `Orbcrypt/`. Comment-aware stripping handles nested block comments
   (see the W2 / I5 disclosure in the workflow file).
3. **No-axiom check** —
...
| `docs/DEVELOPMENT.md` | Master specification (~56KB): full scheme design, security proofs, hardness reductions, 7-stage pipeline. |
...
- `Mathlib.GroupTheory.GroupAction.{Basic, Defs}` — `MulAction`, `orbit`,
  `stabilizer`.
- `Mathlib.GroupTheory.Subgroup.Basic` — `Subgroup` (for G ≤ S_n).
...
- `Mathlib.Analysis.SpecificLimits.Basic` — convergence lemmas for
  negligible functions.

**Version strategy.** Pin to a specific Mathlib4 commit via `lean-toolchain`
and `lakefile.lean` for reproducible builds. Update Mathlib only after
verifying that all proofs still compile.
